...
export default function ProductTable({ products, loading, onDelete }: ProductTableProps) {
  ...
  const handleQRClicked = (uuid: string) => {
    const url = `${window.location.origin}/api/verify/${uuid}`;
    setVerifyUrl(url)
    setOpen(true)
  }
  ...
  return (
    <>
                ...
                currentData.map((product) => (
                  <tr key={product.id} className="group hover:bg-blue-50/50 dark:hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-mono font-bold text-slate-400">#{product.productId}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100/50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                        <QrCode
                          className="w-3 h-3 text-emerald-600 dark:text-emerald-400"
                          onClick={() => handleQRClicked(product.uuid)}
                        />
      ...
      <QRDialog open={open} setOpen={setOpen} verifyUrl={verifyUrl} />
    </>
  );
}